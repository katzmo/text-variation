<template>
  <div class="panel span-2" style="min-height:200px;max-height:240px;">
    <div class="panel-header">
      ...
      <div class="panel-expand" style="margin-left:4px" @click="$emit('expand','Textual similarity')">⤢</div>
      <span style="font-family:var(--mono);font-size:9px;color:var(--ink3);margin-left:6px">2D embedding</span>
    </div>
...
const containerEl = ref(null); const svgEl = ref(null); const tooltipEl = ref(null)
...
function computeMDS(wits) {
  ...
  const B = Array.from({ length: n }, (_, i) => Array.from({ length: n }, (_, j) => -0.5 * (sq[i][j] - rowMean[i] - rowMean[j] + grand)))
...
function draw() {
  ...
  if (!el || !svg) return
  const W = el.clientWidth || 400; const H = el.clientHeight || 190
  ...
  const xScale = d3.scaleLinear().domain([d3.min(xs), d3.max(xs)]).range([0, iW]).nice()
  const yScale = d3.scaleLinear().domain([d3.min(ys), d3.max(ys)]).range([iH, 0]).nice()
  ...
  for (let i = 0; i < n; i++) for (let j = i + 1; j < n; j++) {
    if (wits[i].source === wits[j].source && wits[i].affiliation === wits[j].affiliation)
      g.append('line').attr('x1',xScale(coords[i][0])).attr('y1',yScale(coords[i][1])).attr('x2',xScale(coords[j][0])).attr('y2',yScale(coords[j][1])).attr('stroke','var(--border2)').attr('stroke-width',0.8).attr('opacity',0.5)
  }
  wits.forEach((w, i) => {
    const cx = xScale(coords[i][0]); const cy = yScale(coords[i][1])
    const isSel = w.id === selectedWit.value
    const dg = g.append('g').attr('transform',`translate(${cx},${cy})`).style('cursor','pointer')
      .on('mouseenter', () => { const tip = tooltipEl.value; if (!tip) return; tip.textContent = `${w.id} — ${w.name}`; tip.style.left = (margin.left+cx+8)+'px'; tip.style.top = (margin.top+cy-14)+'px'; tip.classList.add('show') })
      .on('mouseleave', () => tooltipEl.value?.classList.remove('show'))
      .on('click', () => { selectedWit.value = w.id })
    dg.append('circle').attr('r', isSel ? 6 : 4).attr('fill', isSel ? 'var(--ink)' : '#aaa8a0').attr('stroke','#fff').attr('stroke-width', isSel ? 1.5 : 1)
    dg.append('text').attr('x', isSel ? 8 : 6).attr('y', 3.5).attr('font-family','var(--mono)').attr('font-size', isSel ? 9 : 8).attr('font-weight', isSel ? 600 : 400).attr('fill', isSel ? 'var(--ink)' : 'var(--ink2)').text(w.id)
  })
}

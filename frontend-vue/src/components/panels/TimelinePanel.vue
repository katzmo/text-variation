<template>
  <div class="panel span-full" style="min-height: 100px; max-height: 110px">
    ...
    <div
      class="timeline-body"
      ref="containerEl"
      style="flex: 1; position: relative; overflow: hidden; padding: 2px 10px 4px"
    >
...
function draw() {
  ...
  witnesses.value.forEach((w) => {
    ...
    tg.append('line')
      ...
      .attr('x2', 0)
      .attr('y2', axisY)
      .attr('stroke', isSel ? 'var(--ink)' : 'var(--ink3)')
      .attr('stroke-width', isSel ? 2 : 1)
      .attr('opacity', isSel ? 1 : 0.55)
    tg.append('circle')
      .attr('cx', 0)
      .attr('cy', axisY - tickH)
      .attr('r', isSel ? 3 : 1.8)
      .attr('fill', isSel ? 'var(--ink)' : 'var(--ink3)')
      .attr('opacity', isSel ? 1 : 0.6)
  })
}

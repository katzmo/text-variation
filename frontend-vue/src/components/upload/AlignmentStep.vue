    ...
    <div v-else-if="inspectWit" class="align-inspect">
      <div class="align-inspect-header">
        <button
          ...
          @click="inspectWit = null"
        >
          ← Back
        </button>
        <span
          style="font-family: var(--serif); font-size: 13px; font-weight: 700; margin-left: 10px"
        >
          {{ inspectWit }} vs {{ anchorId }}
        </span>
          ...
          <div v-for="(pair, pi) in pairs" :key="pi" class="atv-pair">
            <template v-if="pair.anchor_text">
              <div class="atv-row atv-anchor">
                ...
                <span
                  v-for="(tok, ti) in pair.anchor_tokens"
                  :key="ti"
                  class="atv-tok"
                  :class="{ 'atv-shared': pair.shared.has(tok), 'atv-diff': !pair.shared.has(tok) }"
                >
                  {{ tok }}
                </span>
              </div>
              <div class="atv-row atv-wit">
                ...
                <span
                  v-for="(tok, ti) in pair.wit_tokens"
                  :key="ti"
                  class="atv-tok"
                  :class="{ 'atv-shared': pair.shared.has(tok), 'atv-diff': !pair.shared.has(tok) }"
                >
            ...
            <div v-else class="atv-unaligned">
              <span class="atv-label">{{ inspectWit }}</span>
              <span class="atv-unaligned-text">
                {{ pair.wit_text }}
                <span class="atv-unaligned-badge">unaligned</span>
              </span>
            </div>
          </div>
        </div>
      ...
      <div v-else class="align-stacked-wrap">
        <div
          style="font-size: 11px; color: var(--ink3); margin-bottom: 8px; font-family: var(--mono)"
        >
          Line {{ stackIdx + 1 }}/{{ stackLines.length }}
        </div>
        <input
          type="range"
          :min="0"
          :max="Math.max(0, stackLines.length - 1)"
          v-model.number="stackIdx"
          style="width: 100%; accent-color: var(--ink); margin-bottom: 12px"
        />
        <div class="align-stack-rows" v-if="stackLines.length">
          <div class="asr-anchor">
            <span class="asr-id">{{ anchorId }}</span>
            <span class="asr-text">{{ stackLines[stackIdx]?.anchor_text }}</span>
          </div>
          <div class="asr-row" v-for="row in stackLines[stackIdx]?.witnesses" :key="row.id">
            <span class="asr-id">{{ row.id }}</span>
            <span class="asr-text" :class="{ 'asr-unaligned': !row.aligned }"
              >{{ row.text || '—' }}
              <span v-if="!row.aligned" class="atv-unaligned-badge">unaligned</span>
              <span v-if="row.aligned" class="asr-score">{{ row.score.toFixed(2) }}</span>
            </span>
          </div>
        </div>
      </div>
    ...
    <div v-else>
      <div class="align-controls">
        <!--<div class="align-ctrl-group">
          ...
          <select class="col-settings-select" v-model="anchorId" style="width: 220px">
            <option v-for="w in witList" :key="w.id" :value="w.id">
              {{ w.id }} ({{ w.line_count.toLocaleString() }} lines){{
                w.id === suggested ? ' ★' : ''
              }}
            </option>
          </select>
        ...
        <!--<div class="align-ctrl-group">
          ...
          <div style="display: flex; align-items: center; gap: 8px">
            <span style="font-size: 10px; font-family: var(--mono); color: var(--ink3)">
              permissive
            </span>
            <input
              type="range"
              ...
              style="width: 140px; accent-color: var(--ink)"
            />
            <span style="font-size: 10px; font-family: var(--mono); color: var(--ink3)">
              strict
            </span>
      ...
      <div v-if="results">
        ...
        <!--<table class="align-table">
          ...
          <tbody>
            <tr
              v-for="(stats, wid) in results.witnesses"
              :key="wid"
              :class="{ 'align-row-anchor': wid === results.anchor_id }"
            >
              <td>
                <strong>{{ wid }}</strong>
                <span v-if="wid === results.anchor_id" class="anchor-badge">anchor</span>
              </td>
              ...
              <td>
                <div style="display: flex; align-items: center; gap: 6px">
                  <div class="align-bar-bg">
                    <div class="align-bar-fill" :style="{ width: stats.pct_aligned + '%' }"></div>
                  </div>
              ...
              <td style="font-family: var(--mono); font-size: 11px">
                {{ stats.high_conf?.toLocaleString() }}
              </td>
              <td>
                <button
                  v-if="wid !== results.anchor_id"
                  class="btn-secondary"
...
const threshLabel = computed(() =>
  threshold.value >= 0.7 ? 'strict' : threshold.value >= 0.45 ? 'balanced' : 'permissive',
)
const threshDesc = computed(() => {
  if (threshold.value >= 0.7)
    return 'Only near-identical lines align. Best for clean parallel editions.'
  if (threshold.value >= 0.45)
    return 'Recommended default. Handles spelling variation and minor differences.'
...
async function runAlignment() {
  ...
  try {
    ...
    witList.value.forEach((w) => {
      witnesses_stats[w.id] = {
        total: w.line_count || 1000,
        aligned: Math.round((w.line_count || 1000) * 0.45),
        unaligned: Math.round((w.line_count || 1000) * 0.55),
        pct_aligned: 45.0,
        avg_score: 0.65,
        high_conf: Math.round((w.line_count || 1000) * 0.3),
  ...
  try {
    ...
    pairs.value = data.lines.map((line) => {
      ...
      return {
        ...
        wit_text: line.text,
        score: line.score,
        anchor_tokens: aWords,
        wit_tokens: wWords,
        shared,
      }
    })
    stackLines.value = data.lines
      .filter((l) => l.anchor_text)
      .map((l) => ({
        anchor_text: l.anchor_text,
        witnesses: [{ id: wid, text: l.text, aligned: true, score: l.score }],
      }))
  } catch (e) {
    pairs.value = []
  }
...
async function saveAlignment() {
  try {
    const data = await postSaveAlignment()
    alert(`Saved ${data.count} augmented TEI files.`)
  } catch (e) {
    alert('Save failed — backend not available.')
  }
}
</script>

  ...
  <div class="upload-overlay" @click.self="$emit('close')">
    <div class="upload-modal">
      ...
      <div class="upload-steps">
        <div
          v-for="(label, i) in ['Upload files', 'Edit metadata', 'Load texts', 'Align segments']"
          :key="i"
          class="upload-step"
        ...
        <div v-if="step === 3">
          ...
          <div v-else style="text-align: center; padding: 32px">
            ...
            <div
              ...
              "
            >
              {{ meta.length }} witnesses loaded
            </div>
            <div style="font-size: 12px; color: var(--ink3); font-family: var(--mono)">
              Proceed to align segments, or skip to the dashboard.
            </div>
          </div>
        </div>
      ...
      <div class="upload-footer">
        ...
        <button class="btn-primary" v-if="step === 3 && !processing" @click="goToAlign">
          Align segments →
        </button>
        <button class="btn-secondary" v-if="step === 3 && !processing" @click="$emit('close')">
          Skip — go to dashboard
        </button>
        <button class="btn-primary" v-if="step === 4" @click="$emit('close')">
          Done — view dashboard
        </button>
      </div>
    </div>
...
async function goToMeta() {
  ...
  if (csvFile && csvRows.length >= textFiles.value.length) {
    step.value = 3
    await loadTexts()
  } else step.value = 2
}
...
async function loadTexts() {
  ...
  try {
    ...
    newSegments = meta.value.reduce((a, m) => {
      a[m.id] = m.segments
      return a
    }, {})
  }
  ...
  variants.value = Object.entries(wordFreq)
    .filter(([, c]) => c > 1 && c < meta.value.length * 3)
    .map(([w]) => w)
    .sort()
    .slice(0, 60)

  // Update collation text cache
  Object.entries(newSegments).forEach(([id, segs]) => {
    colTextCache.value[id] = Array.isArray(segs) ? segs : Object.values(segs)
  })
